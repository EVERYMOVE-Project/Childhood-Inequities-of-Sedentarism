...
# predict the total linear predictor for the **log scale** model1
m1_log_total <- predictInterval(model1, which="full", level=0.95, type="linear.prediction")
dt$m1_log_total <- m1_log_total$fit

# CIs for the total effect on logit scale
dt$m1_log_total_upr <- m1_log_total$upr
dt$m1_log_total_lwr <- m1_log_total$lwr

# predict the fixed effect on the **log scale** model 1
m1_log_fixed <- predictInterval(model1, which="fixed", level=0.95, type="linear.prediction")
dt$m1_log_fixed <- m1_log_fixed$fit 

# CIs for the fixed effect on logit scale
dt$m1_log_fixed_upr <- m1_log_fixed$upr
dt$m1_log_fixed_lwr <- m1_log_fixed$lwr

# predict the total effect on the **probability scale** model 1
...
# predict the fixed effect on the **probability scale** model 1
...
stratum_level2 <- dt2 %>% 
  ...
  summarise(
    ...
    across(c(sedentarismo2, strataN, 
             ...
             m1_log_fixed, m1_log_fixed_upr, m1_log_fixed_lwr,
