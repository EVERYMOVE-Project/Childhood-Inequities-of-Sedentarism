...
ense2006 <- ense2006 %>%
  left_join(
    hogar2006 %>% select(id, edad_i, sexo_i, nacionalidad, pais, adultos_hogar, 
...
ense2006 <- ense2006 %>% 
  mutate( 
    ...
    estudios = na_if(estudios, 99, 00),
    education_3 = case_when((
      estudios == 01 | estudios == 02 | estudios == 03) ~ "Low",
      (estudios == 04 | estudios == 05 | estudios == 06 | estudios == 07) ~ "Medium",
      (estudios == 08 | estudios == 09) ~ "High"),
    education_3 = factor(education_3, levels = c("Low", "Medium", "High"), ordered = TRUE),
    education_3_tr = (cume_dist(as.numeric(education_3)) - 1) * -1,
    education_5 = case_when((
      estudios == 01 | estudios == 02) ~ "Low",
      estudios == 03 ~ "Medium-Low",
      (estudios == 04 | estudios == 05) ~ "Medium",
      (estudios == 06 | estudios == 07) ~ "Medium-High",
      (estudios == 08 | estudios == 09) ~ "High"),
