...
rii_sedentarism_overall_clase <- dt %>%
  ...
  group_split() %>%
  map_dfr(~ {
    fit <- glm(sedentarismo ~ clase_tr_3 + edad + sexo, data = .x, family = "poisson")
    coef <- tidy(fit) %>% filter(term == "clase_tr_3")
    tibble(
...
rii_sedentarism_f_clase <- dt_females %>%
  ...
  group_split() %>%
  map_dfr(~ {
    fit <- glm(sedentarismo ~ clase_tr_3 + edad, data = .x, family = "poisson")
    coef <- tidy(fit) %>% filter(term == "clase_tr_3")
    tibble(
      encuesta = unique(.x$survey),
      rii = exp(coef$estimate),
...
sii_sedentarism_overall_clase <- dt %>%
  ...
  map_dfr(~{
    ...
    fit <- glm(
      sedentarismo ~ clase_tr_3 + edad + sexo,
      data = .x,
      family = poisson(link = "identity"),
      start = c(1, 0, 0, 0)  # helps convergence
    )
    
    coef <- broom::tidy(fit) %>% filter(term == "clase_tr_3")
...
table_ccaa <- dt %>%
  ...
  summarise(
    clase_tr_3 = min(table(clase_tr_3)),
    .groups = "drop"
  ) %>%
...
rii_sedentarism_CCAA <- glmmTMB(sedentarismo~clase_tr_3+edad+sexo+
                                  (1+clase_tr_3|survey) + # allows the baseline level of sedentarism and the effect of social class to vary between survey years
...
rii_sedentarism_CCAA_females <- glmmTMB(sedentarismo~clase_tr_3+edad+(1+clase_tr_3|survey) 
                                        + (1+clase_tr_3|survey:ccaa),  data = subset(dt, sexo == "Female"),
...
rii_sedentarism_CCAA_males <- glmmTMB(sedentarismo~clase_tr_3+edad+(1+clase_tr_3|survey) 
                                      + (1+clase_tr_3|survey:ccaa), data = subset(dt, sexo == "Male"),
                                        family="poisson", weights = factor2) # generalized linear mixed model
...
ccaa_survey_table <- dt %>%
  ...
  summarise(
    n = n(),
    median_clase = median(clase_tr_3, na.rm = TRUE),
    p25_clase = quantile(clase_tr_3, 0.25, na.rm = TRUE),
    p75_clase = quantile(clase_tr_3, 0.75, na.rm = TRUE),
    .groups = "drop"
  )
ccaa_survey_table
...
dt %>%
  ...
  arrange(flag_small, survey, ccaa, sexo, clase_tr_3) %>% 
  print(n=1068)
...
dt %>%
  count(survey, ccaa, sexo, clase_tr_3) %>%
  ggplot(aes(x = clase_tr_3, y = n, fill = sexo)) +
  geom_col(position = "dodge") +
  facet_grid(ccaa ~ survey)

sparse_strata <- dt %>%
  group_by(survey, ccaa) %>%
  summarise(min_n = min(table(clase_tr_3)), total_n = n(), .groups = "drop") %>%
  filter(min_n < 15) %>% 
  print(n=51)
...
rii_sedentarism_NUTS1_ccaa <- glmmTMB(
  sedentarismo ~ clase_tr_3 + edad + sexo +
    (1 + clase_tr_3 | survey) +
    (1 + clase_tr_3 | survey:NUTS1) +
    (1 + clase_tr_3 | survey:NUTS1:ccaa),
  data = dt,
  family = poisson,
  weights = factor2
)
...
dt %>%
  ...
  summarise(
    n_classes = n_distinct(clase_tr_3),
    min_n = min(table(clase_tr_3)),
    .groups = "drop"
  ) %>%
