...
ense2003 <- menores2003 %>% select(
  NIDENTIF, SEXO, EDAD, SUJ_ENTR, SPCLASE, FACTOR, ESTAPESO, D_ACFISO,
  PESO, ALTURA, SPLABOR, SPESTUDI, TMUNI, CCAA)

# rename with matching order
...
ense2003 <- ense2003 %>%
  left_join(hogar2003 %>% select(ID, edad_i, nacionalidad, pais, ecivil, ingresos), by = "ID") 
...
ense2003 <- ense2003 %>%
  mutate(
    ...
    percepcion_peso = as.factor(percepcion_peso),
...
ense2006 <- menores2006 %>% select(
  ...
  PESO, ALTURA, MSIMC, SPLABOR, SPESTUDI, ESTRATO, CCAA, A12,
  D3_4, D3_7)
