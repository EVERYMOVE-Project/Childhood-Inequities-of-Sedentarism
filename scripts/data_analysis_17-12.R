...
# Optional: copy to clipboard ## NOTE THAT COLUMNS ARE NOT IN ORDER
...
# View the result ## NOTE TABLE COLUMNS NOT IN RIGHT ORDER
...
dt %>%
  count(ccaa) %>%       # ccaa = English name in your original data
  arrange(n)
...
dt %>%
  count(ccaa, clase) %>%  
  tidyr::pivot_wider(names_from = clase, values_from = n, values_fill = 0) %>%
  arrange(ccaa)

test <- dt %>%
  count(ccaa, clase_tr, sexo) %>%  
  tidyr::pivot_wider(names_from = clase_tr, values_from = n, values_fill = 0) %>%
  arrange(ccaa)
print(test, n = 36)

dt %>%
  filter(sexo == "Female", ccaa == "Galicia", survey == 2023) %>%
  count(clase_tr)

dt %>%
  filter(sexo == "Female", ccaa == "Navarre", survey == 2023) %>%
  count(clase_tr)

dt %>%
  filter(sexo == "Female", ccaa == "Madrid", survey == 2023) %>%
  count(clase_tr)
...
## (1+clase_tr|ccaa) : each region gets its own baseline level of sedentarism,
                    ...
                    # the random part is: 
                        ...
                        # With (1 + clase_tr | ccaa), regions can have different intercepts and different slopes. 
                        # The lines can tilt differently, not just shift up and down.
