...
survey_years <- sort(unique(dt$survey))

rii_sedentarism_overall_clase <-
  map_dfr(survey_years, function(y){
    
    # subset survey design
    des_year <- subset(design, survey == y)
    
    # fit survey-weighted poisson model *quasi to avoid warnings regarding poisson variance assumption with same poisson estimates
    fit <- svyglm(
      sedentarismo ~ clase_tr_2 + edad + sexo,
      design = des_year,
      family = quasipoisson(link = "log")
    )
    
    beta <- coef(fit)["clase_tr_2"]
    se   <- sqrt(vcov(fit)["clase_tr_2","clase_tr_2"])
    
    tibble(
      encuesta = y,
      rii = exp(beta),
      rii_infci = exp(beta - 1.96*se),
      rii_supci = exp(beta + 1.96*se),
...
save(rii_sedentarism_overall_clase, file = "Datasets/clase_tr_2/new2/rii_sedentarism_overall_clase.RData")
...
save(rii_sedentarism_f_clase, file = "Datasets/clase_tr_2/new2/rii_sedentarism_overall_clase_females.RData")
...
save(rii_sedentarism_m_clase, file = "Datasets/clase_tr_2/new2/rii_sedentarism_overall_clase_males.RData")
...
save(rii_sedentarism_clase, file = "Datasets/clase_tr_2/new2/rii_sedentarism.RData")
View(rii_sedentarism_clase)
...
save(sii_sedentarism_overall_clase, file = "Datasets/clase_tr_2/new2/sii_sedentarism_overall_clase.RData")
...
sii_sedentarism_f_clase <- dt_females %>%
  ...
  map_dfr(~ {
    ...
    # Fit Poisson model
    fit <- glm(sedentarismo ~ clase_tr_2 + edad,
               data = .x, family = poisson(link = "identity"),
               weights = factor2,
               start = c(1, 0, 0))
...
save(sii_sedentarism_f_clase, file = "Datasets/clase_tr_2/new2/sii_sedentarism_clase_females.RData")
...
save(sii_sedentarism_m_clase, file = "Datasets/clase_tr_2/new2/sii_sedentarism_clase_males.RData")
...
save(sii_sedentarism_clase, file = "Datasets/clase_tr_2/new2/sii_sedentarism.RData")
...
save(inequalities_sedentarism, file = "Datasets/clase_tr_2/new2/inequalities_sedentarism.RData")

#### Visualization of RII in Sedentarism ####
load("Datasets/clase_tr_2/new2/inequalities_sedentarism.RData")
...
fig_rii_class_sex
ggsave("Figures/clase_tr_2/new2/fig_rii_class_sex.png", width = 4000, height = 2200, dpi=300, units = "px")
...
fig_sii_class_sex
ggsave("Figures/clase_tr_2/new2/fig_sii_class_sex.png", width = 4000, height = 2200, dpi=300, units = "px")
...
fig_sii_rii
ggsave("Figures/clase_tr_2/new2/fig_sii_rii.png", width = 4000, height = 2200, dpi=300, units = "px")
...
rii_sedentarism_NUTS1_females <- glmmTMB(sedentarismo~clase_tr_2+edad_scale+
                                           (1+clase_tr_2|survey) 
                                        + (1+clase_tr_2|survey:NUTS1),  
...
rii_sedentarism_NUTS1_males <- glmmTMB(sedentarismo~clase_tr_2+edad_scale+
                                         (1+clase_tr_2|survey) 
                                       + (1+clase_tr_2|survey:NUTS1), 
...
## label data
label_data <- rii_sedentarism_NUTS1_combined

## Plots by NUTS region
plots_by_region <- rii_sedentarism_NUTS1_combined %>%
  split(.$NUTS1) %>%
  map(~ ggplot(.x,
               aes(x = as.factor(survey),
                   y = rii,
                   group = sex,
                   color = sex,
                   fill = sex)) +
        ...
        geom_ribbon(aes(ymin = rii_infci, ymax = rii_supci),
                    alpha = 0.15, color = NA) +
        ...
        labs(
          title = paste("Sedentarism inequalities in the NUTS region:", unique(.x$NUTS1)),
          x = "Survey Year",
          y = "RII (95% CI)"
        ) +
...
plots_by_region  
ggsave("Figures/clase_tr_2/RII_CanaryIslands.png", width = 4000, height = 2200, dpi=300, units = "px")

# Figure all NUTS regions over time
apc_labels <- rii_sedentarism_NUTS1_combined %>%
  filter(sex == "Overall") %>%
  group_by(NUTS1) %>%
  filter(survey == max(survey)) %>%
  ungroup() %>%
  left_join(apc_nuts, by = "NUTS1")

apc_labels

ggplot(rii_sedentarism_NUTS1_combined %>% 
         dplyr::filter(sex == "Overall"),
       
       aes(x = survey, y = rii, group = NUTS1, color = NUTS1)) +
  
  geom_hline(yintercept = 1, linetype = "dashed", color = "gray40") +
  
  geom_ribbon(aes(ymin = rii_infci, ymax = rii_supci),
              alpha = 0.15, color = NA, fill = "grey60") +
  geom_text_repel(
    data = apc_labels,
    aes(
      label = paste0(
  ...
  labs(
    title = "Trends in Socioeconomic Inequalities in Sedentarism",
    subtitle = "RII over time by NUTS region",
    x = "Survey Year",
    y = "RII",
    color = "NUTS region"
  ) +
...
ggsave("Figures/clase_tr_2/RII_NUTS_Overall.png", width = 4000, height = 2200, dpi=300, units = "px")
...
rii_map_survey <- ggplot(
  ...
  labs( title = "Spatial representation of socioeconomic inequalities in sedentarism by NUTS region per survey year",
...
ggsave(
  filename = "Figures/clase_tr_2/new/RII_map_overall.png",
  plot = rii_map_survey,
  width = 13.3,
  height = 7.3,
  dpi = 300,
...
rii_map_survey_NUTS_f <- ggplot(
  ...
  labs( title = "Sedentarism inequalities among girls by NUTS region per survey year",
        subtitle = "Unit: Relative Index of Inequality",
        caption = "Nomenclature of Territorial Units for Statistics (NUTS) Regions",
        fill = "Relative Index of Inequality") +
  theme_map()+
...
ggsave(
  filename = "Figures/clase_tr_2/new/RII_map_overall_girls.png",
...
rii_map_survey_NUTS_m <- ggplot(
  ...
  labs( title = "Sedentarism inequalities among boys by NUTS region per survey year",
        subtitle = "Unit: Relative Index of Inequality",
        caption = "Nomenclature of Territorial Units for Statistics (NUTS) Regions",
        fill = "Relative Index of Inequality") +
  theme_map()+
...
ggsave(
  filename = "Figures/clase_tr_2/new/RII_map_overall_boys.png",
...
rii_sedentarism_CCAA_females <- glmmTMB(sedentarismo~clase_tr_2+edad_scale+(1+clase_tr_2|survey) 
...
rii_sedentarism_CCAA_males <- glmmTMB(sedentarismo~clase_tr_2+edad_scale+(1+clase_tr_2|survey) 
                                      + (1+clase_tr_2|survey:ccaa), data = subset(dt, sexo == "Male"),
                                      family="poisson", weights = factor2) # generalized linear mixed model
...
save(rii_sedentarism_CCAA_males_t, file = "Datasets/clase_tr_2/new/rii_sedentarism_CCAA_males.RData")
...
load("Datasets/clase_tr_2/new2/rii_sedentarism_overall_clase.RData")
...
ggsave("Figures/clase_tr_2/inflection_20-07-26.png", width = 4000, height = 2200, dpi=300, units = "px")

## Annual Percent Change ----
apc_dta_overall <- rii_sedentarism_clase %>%
  filter(
    strata == "Boys" # Change Overall, Girls, Boys
  ) %>%
  mutate(
    encuesta = as.integer(encuesta)
  )

apc_overall <- apc_dta_overall %>%
  summarise(
    model = list(rlm(log(est) ~ encuesta, data = cur_data()))
  ) %>%
  mutate(
    tidy = map(model, tidy)
  ) %>%
  unnest(tidy) %>%
  filter(term == "encuesta") %>%
  mutate(
...
apc_dta <- rii_sedentarism_CCAA_combined %>%
  filter(
    ...
    sex == "Overall" # Change Overall, Girls, Boys
  ) %>%
  mutate(
    survey = as.integer(survey)
  )
